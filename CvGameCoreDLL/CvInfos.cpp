...
int CvHandicapInfo::getUnitCostPercentByID(PlayerTypes pl) const		
{
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (pl<NUM_MAJOR_PLAYERS)
			result += (result*(startingTurn[pl] - GET_PLAYER((PlayerTypes)pl).getBirthTurn())/600);
	}
...
int CvHandicapInfo::getResearchPercentByID(PlayerTypes pl) const		
{
	...
	if (GC.getGameINLINE().getHandicapType() <= 1) { //viceroy or lower (RFCRAND)
		basePercent = 50;//61; //59 in vanilla and Warlords
		humanContributionPercent = 105;
	}
	else if (GC.getGameINLINE().getHandicapType() >= 3) { //emperor or higher (RFCRAND)
	...
	if (GC.getGameINLINE().getGameTurn() >= GET_PLAYER((PlayerTypes)GC.getGameINLINE().getActivePlayer()).getBirthTurn()) { //RFCRAND
	...
	if (GC.getGameINLINE().getGameTurn() < 75 ) 
		{
			researchPercent *= 105;
			researchPercent /= 100;
		}
	else if (GC.getGameINLINE().getGameTurn() >=75 && GC.getGameINLINE().getGameTurn() <= 180) //up to where the new timeline's stretched
		{
			researchPercent *= 150;
			researchPercent /= 100;
		}
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (pl<NUM_MAJOR_PLAYERS)
			researchPercent += (researchPercent*(startingTurn[pl] - GET_PLAYER((PlayerTypes)pl).getBirthTurn())/500);
	}
	
	int result = researchPercent;

	switch (pl)
	{
	case EGYPT:
		result = researchPercent*135/100; //125 before the new timeline
	case INDIA:
		result = researchPercent*135/100; //123 before the new timeline
	case CHINA:
		result = researchPercent*122/100; //115 before the new timeline
	case BABYLONIA:
		if (GET_PLAYER((PlayerTypes)pl).getCurrentEra() <= 1)
			result = researchPercent*120/100; //a little boost for the UHV
		else
			result = researchPercent*127/100; //RFCRAND (125 in RFC)   //113 before the new timeline   
	case GREECE:
		result = researchPercent*120/100; //RFCRAND (116 in RFC)      //102 before the new timeline +higher because the UP already helps
	case PERSIA:
		result = researchPercent*103/100; //RFCRAND (100 in RFC) //90 before the new timeline
	case CARTHAGE:
		result = researchPercent*100/100; //90 before the new timeline
	case ROME:
		result = researchPercent*113/100; //RFCRAND (107 in RFC) //95 before the new timeline
	case JAPAN:
		result = researchPercent*98/100; //95 before the new timeline //120 before embassies
	case ETHIOPIA:
		result = researchPercent*110/100; //105 before the new timeline
	case MAYA:
		//Rhye - start UP (Mayan)
		//result = researchPercent*90/100;
		if (GET_PLAYER((PlayerTypes)pl).getCurrentEra() >= 1 && GET_PLAYER((PlayerTypes)pl).getCurrentEra() <= 2) //RFCRAND
			result = researchPercent*65/100;
		else
			result = researchPercent*120/100;  //RFCRAND (115 in RFC)
		//Rhye - end UP		
	case VIKING:
		result = researchPercent*8/10;
	case GERMANY:
		result = researchPercent*69/100;
	case FRANCE:
		result = researchPercent*78/100;    //RFCRAND (80 in RFC)
	case ARABIA:
		result = researchPercent*100/100;
	case KHMER:
		result = researchPercent*100/100;
	case SPAIN:
		result = researchPercent*78/100;  //RFCRAND (80 in RFC)
	case ENGLAND:
		result = researchPercent*78/100;  //RFCRAND (80 in RFC)
	case RUSSIA:
		result = researchPercent*67/100; //should be 75, they need a lower value cos their empire size and lack of commerce make them very slow
	case NETHERLANDS:
		result = researchPercent*74/100;  //RFCRAND (75 in RFC)
	case MALI:
		result = researchPercent*115/100; //RFCRAND (110 in RFC) //140 before No tech brokering  //high to counter their UP
	case TURKEY:
		result = researchPercent*75/100;
	case PORTUGAL:
		result = researchPercent*74/100;  //RFCRAND (75 in RFC)
	case INCA:
		result = researchPercent*79/100;
	case MONGOLIA:
		result = researchPercent*73/100;
	case AZTEC:
		result = researchPercent*87/100;
	case AMERICA:
		result = researchPercent*63/100;
	case CELTIA: 
		if (!GET_PLAYER((PlayerTypes)EGYPT).isPlayable()) //late start condition
			result = researchPercent*350/100; //Byzantium
		else
			result = researchPercent*100/100; //Celts
	default:
		result = researchPercent*100/100; //RFCRAND
		break;
	}
	
	//Rhye RFCRAND
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		//normalization with a line
		if (pl<NUM_MAJOR_PLAYERS)
			result = (result+(350-GET_PLAYER((PlayerTypes)pl).getBirthTurn())*(130-63)/350+63)/2; 
		//all higher, randomizing makes pace too fast
		result = result*109/100;
	}
...
int CvHandicapInfo::getCivicUpkeepPercentByID(PlayerTypes pl) const	
{
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (pl<NUM_MAJOR_PLAYERS)
			result += (result*(startingTurn[pl] - GET_PLAYER((PlayerTypes)pl).getBirthTurn())/500);
	}
...
int CvHandicapInfo::getHealthBonusByID(PlayerTypes pl) const			
{
	...
	case GREECE:
		...
		break;
	case PERSIA:
		result = result*5;   //4 before removal of Expansive trait
		break;
	case CARTHAGE:
		result = result*7;
		break;
	case ROME:
		result = result*7;   //4 before removal of Expansive trait
		break;
	case JAPAN:
		result = result*3;
		break;
	...
	case FRANCE:
		...
		break;
	case SPAIN:
		result = result*6;   //4 before removal of Expansive trait
		break;
	case ENGLAND:
		result = result*6;   //4 before removal of Expansive trait
		break;
	...
	case INCA:
		...
		break;
	case MONGOLIA:
		result = result*6;  //7 //4 before removal of Expansive trait
		break;
	case AZTEC:
		result = result*4;
		break;
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (pl<NUM_MAJOR_PLAYERS)
			result += int((GET_PLAYER((PlayerTypes)pl).getBirthTurn() - startingTurn[pl])/75);
	}
