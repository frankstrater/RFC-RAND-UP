...
#include "CvRhyes.h" //Rhye - RFCRAND
...
void CvInitCore::resetPlayer(PlayerTypes eID, CvInitCore * pSource, bool bClear, bool bSaveSlotInfo)
{
	...
	if ( checkBounds(eID, 0, MAX_PLAYERS) )
	{
		...
		if (pSource)
		{
			...
			/**/
			//setCivDescription(eID, pSource->getCivDescription((PlayerTypes)assignedCiv[eID]));
			//setCivShortDesc(eID, pSource->getCivShortDesc((PlayerTypes)assignedCiv[eID]));
			//setCivAdjective(eID, pSource->getCivAdjective((PlayerTypes)assignedCiv[eID]));

			/*setCivPassword(eID, pSource->getCivPassword((PlayerTypes)assignedCiv[eID]), false);
			setEmail(eID, pSource->getEmail((PlayerTypes)assignedCiv[eID]));
			setSmtpHost(eID, pSource->getSmtpHost((PlayerTypes)assignedCiv[eID]));
			//setFlagDecal(eID, pSource->getFlagDecal((PlayerTypes)assignedCiv[eID]));
			setWhiteFlag(eID, pSource->getWhiteFlag((PlayerTypes)assignedCiv[eID]));

			setHandicap(eID, pSource->getHandicap((PlayerTypes)assignedCiv[eID]));

			//setCiv(eID, (CivilizationTypes)assignedCiv[iLoopCiv]);
			//setTeam(eID, (TeamTypes)assignedCiv[iLoopCiv]);
			//setLeader(eID, assignedCiv[iLoopCiv]);
			//setColor(eID, assignedCiv[iLoopCiv]);
			setCiv(eID, (CivilizationTypes)assignedCiv[eID]);
			setTeam(eID, (TeamTypes)assignedCiv[eID]);
			setLeader(eID, pSource->getLeader((PlayerTypes)assignedCiv[eID]));
			setColor(eID, pSource->getColor((PlayerTypes)assignedCiv[eID]));
			setArtStyle(eID, pSource->getArtStyle((PlayerTypes)assignedCiv[eID]));
			...
			/*if (eID == 0) {
				for (int iLoopCiv = 0; iLoopCiv < NUM_MAJOR_PLAYERS; iLoopCiv++)
					if (getCiv(eID) == (CivilizationTypes)assignedCiv[iLoopCiv]) {
						m_aeSlotStatus[eID] = SS_COMPUTER;
						m_aeSlotStatus[iLoopCiv] = SS_TAKEN;
						setCiv(eID, (CivilizationTypes)assignedCiv[0]);
					}
			}*/
			if (eID >= NUM_MAJOR_PLAYERS && eID != BARBARIAN)
				setMinorNationCiv(eID, true);
			else
				setMinorNationCiv(eID, false);
			//Rhye - end
			...
			if (!bSaveSlotInfo)
			{
				...
				setLeaderName(eID, pSource->getLeaderName(eID)); //Rhye (RFCRAND)
				//setLeaderName(eID, GC.getLeaderHeadInfo((LeaderHeadTypes)assignedEarlyLeader[eID]).getDescription(0));  //Rhye (RFCRAND)
...
void CvInitCore::setActivePlayer(PlayerTypes eActivePlayer)
{
	//Rhye RFCRAND - start 
	/*for (int iLoopCiv = 0; iLoopCiv < NUM_MAJOR_PLAYERS; iLoopCiv++)
		if (getCiv(eActivePlayer) == (CivilizationTypes)assignedCiv[iLoopCiv])
			eActivePlayer = (PlayerTypes)iLoopCiv;*/
	//Rhye - end
