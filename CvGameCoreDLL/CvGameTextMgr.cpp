...
//			}
	...
	if (!pPlot->isWater())
	{
		...
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getBlindSettlersMap", argsList.makeFunctionArgs(), &result);
		int iResult = (int)result;
		szString.append(gDLL->getText("TXT_KEY_STABILITY_ADVISOR_TITLE"));
		
		bool bNearCapital = false;
			for (int iLoopCiv=0;iLoopCiv < NUM_MAJOR_PLAYERS; iLoopCiv++) {
				if (iLoopCiv != GC.getGameINLINE().getActivePlayer() && GET_PLAYER((PlayerTypes)iLoopCiv).isEverAlive()) { 

					/*char buf[30];
					sprintf(buf, "iLoopCiv: %d;everalive: %d;GET_PLAYER((PlayerTypes)iLoopCiv).getStartingPlot(): %d;", iLoopCiv,GET_PLAYER((PlayerTypes)iLoopCiv).isEverAlive(),GET_PLAYER((PlayerTypes)iLoopCiv).getStartingPlot());
					GC.getGameINLINE().logMsg(buf);*/

					CvPlot* pLoopStartingPlot = GET_PLAYER((PlayerTypes)iLoopCiv).getStartingPlot();
					if (pLoopStartingPlot != NULL)

						if (std::max(abs(pPlot->getX()-pLoopStartingPlot->getX()),abs(pPlot->getY()-pLoopStartingPlot->getY())) <= 4) {
							bNearCapital = true;
							break;
						}
				}
		...
		if (std::max(abs(pPlot->getX()-GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getStartingPlot()->getX()),abs(pPlot->getY()-GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getStartingPlot()->getY())) <= 5) {		
			if (bNearCapital == false)
					szString.append(gDLL->getText("TXT_KEY_SEALEVEL_HIGH"));
				else
					szString.append(gDLL->getText("TXT_KEY_SEALEVEL_MEDIUM"));
		}
		else {
			if (iResult < 70) {	
				if (bNearCapital == false)
					szString.append(gDLL->getText("TXT_KEY_SEALEVEL_LOW"));
				else
					szString.append(gDLL->getText("TXT_KEY_SEALEVEL_VERYLOW"));
			}
			else {
				if (bNearCapital == false)
					szString.append(gDLL->getText("TXT_KEY_SEALEVEL_MEDIUM"));
		...
		if (GC.getGameINLINE().isDebugMode()) {
			CvWString szTempBuffer;
			szTempBuffer.Format(L"%d", iResult);
			
			szString.append(" (");
			//szString.append(iResult);
			szString.append(szTempBuffer);
			szString.append(")");			
		}
	}	
...
//Rhye - lots of stuff to update here - leaving wchar, as in v14
	...
	if (eCivilization != NO_CIVILIZATION)
	{
		...
		if (GC.getMapINLINE().getSeaLevel() != 4) { //RFCRAND
		...
		if (GC.getMapINLINE().getSeaLevel() != 4 || !bDawnOfMan) { //RFCRAND
			szInfoText.append(szTempString);
