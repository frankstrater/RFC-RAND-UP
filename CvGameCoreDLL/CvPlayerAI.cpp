					...
					if (!(GET_TEAM(getTeam()).isHuman()) && (GET_PLAYER((PlayerTypes)iI).isHuman() || !(GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHuman())))
					{
						if (GET_TEAM(getTeam()).isAtWar(GET_PLAYER((PlayerTypes)iI).getTeam()))
						{
							if (!(GET_PLAYER((PlayerTypes)iI).isHuman()) || (GET_TEAM(getTeam()).getLeaderID() == getID()))
							{
								...
								{
									//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_PEACE_TREATY) == 0) //Rhye - RFCRAND (workaround for contacts bug)
									if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_PEACE_TREATY) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
									{
										bool bOffered = false;
										...
										if (canTradeItem((PlayerTypes)iI, item, true))
										{
											...
											if (GET_PLAYER((PlayerTypes)iI).isHuman())
											{
												if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
												{
													...
													m_iAdditionalContactTimer = 8; //Rhye - RFCRAND (workaround for contacts bug)
...
int CvPlayerAI::AI_foundValue(int iX, int iY, int iMinRivalRange, bool bStartingLoc) const
{
	...
	if (bDebugAll) {
		sprintf(buf, "Test 0 (%d,%d) iBadTile: %d;", iX, iY, iBadTile);
	...
	//Rhye - comment? (RFCRAND) causes crash
	...
	//Rhye RFCRAND - start
	if (bDebugAll) {
		sprintf(buf, "Test 1 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	//Rhye RFCRAND - start
	if (bDebugAll) {
		sprintf(buf, "Test 2 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	case EGYPT:
		...
		break;
	case INDIA:
		if (iTakenTiles > (NUM_CITY_PLOTS /2 ))
		{
			return 0;
		}
		break;
	...
	case ETHIOPIA:
		...
		break;
	case MAYA:
		if (iTakenTiles > (NUM_CITY_PLOTS *2/3 +1)) //-1??
		{
			return 0;
		}
		break;
	...
	case GERMANY:
		...
		break;
	case RUSSIA:
		if (iTakenTiles > (NUM_CITY_PLOTS /3 +1)) 
		{
			return 0;
		}
		break;
	...
	//Rhye RFCRAND - start
	if (bDebugAll) {
		sprintf(buf, "Test 3 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	if (bStartingLoc)
	{
		...
		//Rhye RFCRAND - start
		if (bDebugAll) {
			sprintf(buf, "Test 3a (%d,%d): %d;", iX, iY, iValue);
			GC.getGameINLINE().logMsg(buf);
			sprintf(buf, "Test 3a (%d,%d) iGreaterBadTile: %d;", iX, iY, iGreaterBadTile);
			GC.getGameINLINE().logMsg(buf);
		}
		//Rhye - end
		...
		//Rhye RFCRAND - start
		if (bDebugAll) {
			sprintf(buf, "Test 3b (%d,%d): %d;", iX, iY, iValue);
			GC.getGameINLINE().logMsg(buf);
		}
		//Rhye - end
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 4 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	if (bStartingLoc)
	{
		...
		{
			//Rhye RFCRAND - start comment
			/*iValue *= (1 + 4 * pPlot->getMinOriginalStartDist());
			iValue /= (1 + 2 * GC.getMapINLINE().maxStepDistance());*/
			//Rhye RFCRAND - end comment
			iValue += std::max(7,pPlot->getMinOriginalStartDist()) * 130; //restored Warlords function
		}

		//Rhye RFCRAND - start
		if (bDebugAll) {
			sprintf(buf, "Test 4a (%d,%d): %d; minOrigStartDist: %d", iX, iY, iValue, pPlot->getMinOriginalStartDist());
			GC.getGameINLINE().logMsg(buf);
		}
		//Rhye - end
		...
		if (!pPlot->isStartingPlot())
		{
			...
			//Rhye RFCRAND - start
			if (bDebugAll) {
				sprintf(buf, "Test 4b0 (%d,%d): %d; iMinDistanceFactor: %d", iX, iY, iValue, iMinDistanceFactor);
				GC.getGameINLINE().logMsg(buf);
			}
			//Rhye - end
			...
			//Rhye RFCRAND - start
			for (int iJ = 0; iJ < getID(); iJ++)
			{
				CvPlot* pLoopStartingPlot = GET_PLAYER((PlayerTypes)iJ).getStartingPlot();
				if (pLoopStartingPlot != NULL)
				{
					int dist = stepDistance(iX, iY, pLoopStartingPlot->getX_INLINE(), pLoopStartingPlot->getY_INLINE());
						if (dist <= 3)
							return 0;
						if (dist == 4) {
							iValue *= 5;
			...
			//Rhye RFCRAND - start
			if (bDebugAll) {
				sprintf(buf, "Test 4b1 (%d,%d): %d; iMinDistanceFactor: %d", iX, iY, iValue, iMinDistanceFactor);
				GC.getGameINLINE().logMsg(buf);
			}
			//Rhye - end

			//Rhye - RFCRAND (for the 3684986582 values in America)
			iMinDistanceFactor = std::min(700, iMinDistanceFactor);
			//Rhye - RFCRAND comment
			...
			else*/ if (iMinDistanceFactor < 1000) //Rhye - RFCRAND comment
			{
			...
			//Rhye RFCRAND - start
			if (bDebugAll) {
				sprintf(buf, "Test 4c (%d,%d): %d;", iX, iY, iValue);
				GC.getGameINLINE().logMsg(buf);
			}
			//Rhye - end
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 5 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	if (pNearestCity != NULL)
	{
		if (isBarbarian())
		{
			...
			//Rhye RFCRAND - start
			if (bDebugAll) {
				sprintf(buf, "Test 5a (%d,%d): %d;", iX, iY, iValue);
				GC.getGameINLINE().logMsg(buf);
			}
			//Rhye - end
		}
		else
		{
			...
			*/
			...
			case CHINA:
				...
				break;
			case BABYLONIA:
				iValue -= (abs(iDistance) - 2) * 500;
				break;
			case GREECE:
				iValue -= (abs(iDistance) - 2) * 300;
				break;
			case PERSIA:
				iValue -= (abs(iDistance) - 3) * 500;
				break;
			...
			//Rhye RFCRAND - start
			if (bDebugAll) {
				sprintf(buf, "Test 5b (%d,%d): %d;", iX, iY, iValue);
				GC.getGameINLINE().logMsg(buf);
			}
			//Rhye - end
			...
			else if (getCapitalCity() != NULL)
			{
				...
				//Rhye RFCRAND - start
				if (bDebugAll) {
					sprintf(buf, "Test 5b2 (%d,%d): %d;", iX, iY, iValue);
					GC.getGameINLINE().logMsg(buf);
				}
				//Rhye - end
			}
	...
	else
	{
		...
		//Rhye RFCRAND - start
		if (bDebugAll) {
			sprintf(buf, "Test 5c (%d,%d): %d;", iX, iY, iValue);
			GC.getGameINLINE().logMsg(buf);
		}
		//Rhye - end
	}
	...
	//Rhye RFCRAND - start
	if (bDebugAll) {
		sprintf(buf, "Test 6 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	//Rhye RFCRAND - start
	if (bDebugAll) {
		sprintf(buf, "Test 7 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	//Rhye RFCRAND - start
	if (bDebugAll) {
		sprintf(buf, "Test 8 (%d,%d): %d;", iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	//if (!(iX >= 67 && iX <= 70 && iY >= 43 && iY <= 46) && !(iX >= 79 && iX <= 18 && iY >= 110 && iY <= 10)) { //Rhye (exclude Turkey and Siberia) RFCRAND
	if (bStartingLoc)
	{
	...
	if (!bStartingLoc) {
		...
		for (int iI = 0; iI < NUM_DIRECTION_TYPES; ++iI) {
			...
			if (pLoopPlot != NULL) {
				if (pLoopPlot->isWater()) {
					if (pLoopPlot->area()->getID() != firstID && firstID == -1)
						firstID = pLoopPlot->area()->getID();
					if (pLoopPlot->area()->getID() != firstID) { //strait
						iValue *= 3;
						break;
					}
				}
	...
	//char buf[30];
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 9 Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	//iValue *= AI_blindSettlersMap(iX, iY, bStartingLoc);
	//iValue /= 100;

	//-----------
	

	if (getID() >= NUM_MAJOR_PLAYERS || iValue < 50)
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10a Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	int iMinPercent = 30 + GC.getMapINLINE().getSeaLevel();
	int iDecreaseModifier = 2;
	int iBonusPercent = 120 - GC.getMapINLINE().getSeaLevel();
	int iMinLatitude = settlersCoords[getID()][2];
	int iMaxLatitude = settlersCoords[getID()][3];
	if (bStartingLoc) {
		iMinPercent = 10 + GC.getMapINLINE().getSeaLevel()*3;
		iDecreaseModifier = 5;
		iBonusPercent = 150 - GC.getMapINLINE().getSeaLevel()*2;
		iMinLatitude = settlersCoords[getID()][0];
		iMaxLatitude = settlersCoords[getID()][1];
	}
	if (GC.getMapINLINE().getClimate() == 3) { //cold
		iMinLatitude = std::max(0, std::min(90, iMinLatitude/2));
		iMaxLatitude = std::max(0, std::min(90, iMaxLatitude*2/3)); 
	}
	else if (GC.getMapINLINE().getClimate() == 1 || GC.getMapINLINE().getClimate() == 2)  { //tropical/arid}
		iMinLatitude = std::max(0, std::min(90, iMinLatitude*5/4));
		iMaxLatitude = std::max(0, std::min(90, iMaxLatitude*4/3)); 
	}
	/*else if (GC.getMapINLINE().getClimate() == 3) { //rocky
		iMinLatitude = std::max(0, std::min(90, iMinLatitude*4/5));
		iMaxLatitude = std::max(0, std::min(90, iMaxLatitude*5/6)); 
	}*/

	if (pPlot->getRealLatitude() <= iMinLatitude) {
		iValue *= std::max(iMinPercent,(100 - iDecreaseModifier*(iMinLatitude-pPlot->getRealLatitude())));
		iValue /= 100;
	}
	else if (pPlot->getRealLatitude() >= iMaxLatitude) {
		iValue *= std::max(iMinPercent,(100 - iDecreaseModifier*(pPlot->getRealLatitude()-iMaxLatitude)));
		iValue /= 100;
	}
	else if (pPlot->getRealLatitude() >= (iMinLatitude+((iMaxLatitude-iMinLatitude)/3)) && 
		pPlot->getRealLatitude() <= (iMinLatitude+(2*(iMaxLatitude-iMinLatitude)/3)) ) {
		iValue *= iBonusPercent;
		iValue /= 100;
	}

	CvPlot* pStartingPlot = getStartingPlot();
	if (!bStartingLoc) { //longitude
		int iMaxExtension = settlersCoords[getID()][4]*GC.getMapINLINE().getGridWidthINLINE()/100;
		if (pStartingPlot != NULL)
			{
			if ((iX + GC.getMapINLINE().getGridWidthINLINE() >= pStartingPlot->getX() + iMaxExtension + GC.getMapINLINE().getGridWidthINLINE())) {
				iValue *= std::max(60, 100 - (iX - (pStartingPlot->getX() + iMaxExtension)));
				iValue /= 100;
			}
			else if ((iX + GC.getMapINLINE().getGridWidthINLINE() <= pStartingPlot->getX() - iMaxExtension + GC.getMapINLINE().getGridWidthINLINE()) ) {
				iValue *= std::max(60, 100 - ((pStartingPlot->getX() + iMaxExtension) - iX));
				iValue /= 100;
			}
		}
	}
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10b Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	if (bStartingLoc) { //clustering
		int iMaxHeight = GC.getMapINLINE().getGridHeightINLINE();
		int iClusteringValue = 100;
		int iSmallRadius = iMaxHeight / 7; // 9/8						/ 9; // 7/6
		int iLargeRadius = iMaxHeight / 4; // 17/14						/ 5; // 13/11
		int iGreaterSmallRadius = iMaxHeight / 5; // 13/11				/ 7; // 9/8
		int iGreaterLargeRadius = iMaxHeight / 3; // 22/18				/ 4; // 17/14
		//int iPositiveModifier = 150;
		//int iGreaterPositiveModifier = 200;
		//int iNegativeModifier = 62;
		//int iGreaterNegativeModifier = 25;
		for (int iLoopCiv = 0; iLoopCiv < getID(); iLoopCiv++) {	
			CvPlot* pLoopCivStartingPlot = GET_PLAYER((PlayerTypes)iLoopCiv).getStartingPlot();
			if (pLoopCivStartingPlot != NULL) {
				int iLoopCivX = pLoopCivStartingPlot->getX();
				int iLoopCivY = pLoopCivStartingPlot->getY();
				int iDist = std::max(abs(iX-iLoopCivX), abs(iY-iLoopCivY));

				int iPositiveModifier = 150;
				int iGreaterPositiveModifier = 200;
				int iNegativeModifier = 62;
				int iGreaterNegativeModifier = 20;

				bool bOtherCity = false;
				int iLoop;
				for (CvCity* pLoopCity = GET_PLAYER((PlayerTypes)iLoopCiv).firstCity(&iLoop); NULL != pLoopCity; pLoopCity = GET_PLAYER((PlayerTypes)iLoopCiv).nextCity(&iLoop))
				{
					int iLoopCityX = pLoopCity->getX_INLINE();
					int iLoopCityY = pLoopCity->getY_INLINE();
					int iDistCity = std::max(abs(iX-iLoopCityX), abs(iY-iLoopCityY));
					if (iDistCity < iDist)
					{
						iDist = iDistCity;
						bOtherCity = true;
				...
				if (bOtherCity) {
					iPositiveModifier = 100 + (iPositiveModifier-100)*50/100;
					iGreaterPositiveModifier = 100 + (iGreaterPositiveModifier-100)*50/100;
					iNegativeModifier = 100 - (100-iNegativeModifier)*50/100;
					iGreaterNegativeModifier = 100 - (100-iGreaterNegativeModifier)*50/100;
				}

				if (startingLocationsClustering[getID()][iLoopCiv] == 2) {
					if (iDist <= iGreaterSmallRadius) {
						iClusteringValue *= iGreaterPositiveModifier;
						iClusteringValue /= 100;
					}
					else if (iDist <= iGreaterLargeRadius) {
						iClusteringValue *= 100 + (iGreaterLargeRadius-iDist)*((iGreaterPositiveModifier-100)/(iGreaterLargeRadius-iGreaterSmallRadius));
						iClusteringValue /= 100;
					}
				}
				else if (startingLocationsClustering[getID()][iLoopCiv] == 1) {
					if (iDist <= iSmallRadius) {
						iClusteringValue *= iPositiveModifier;
						iClusteringValue /= 100;
					}
					else if (iDist <= iLargeRadius) {
						iClusteringValue *= 100 + (iLargeRadius-iDist)*((iPositiveModifier-100)/(iLargeRadius-iSmallRadius));
						iClusteringValue /= 100;
					}
				}
				else if (startingLocationsClustering[getID()][iLoopCiv] == -1) {
					if (iDist <= iSmallRadius) {
						iClusteringValue *= iNegativeModifier;
						iClusteringValue /= 100;
					}
					else if (iDist <= iLargeRadius) {
						iClusteringValue *= iNegativeModifier + (iDist-iSmallRadius)*((100-iNegativeModifier)/(iLargeRadius-iSmallRadius));
						iClusteringValue /= 100;
					}
				}
				else if (startingLocationsClustering[getID()][iLoopCiv] == -2) {
					if (iDist <= iGreaterSmallRadius) {
						iClusteringValue *= iGreaterNegativeModifier;
						iClusteringValue /= 100;
					}
					else if (iDist <= iGreaterLargeRadius) {
						iClusteringValue *= iGreaterNegativeModifier + (iDist-iGreaterSmallRadius)*((100-iGreaterNegativeModifier)/(iGreaterLargeRadius-iGreaterSmallRadius));
						iClusteringValue /= 100;
					}
				}
			}
			//Rhye RFCRAND - start
			if (bDebugSL) {
				sprintf(buf, "iClusteringValue at %d: %d", iLoopCiv, iClusteringValue);
				GC.getGameINLINE().logMsg(buf);
			}
			//Rhye - end
		...
		int iClusteringStrength = 120;
		if (GC.getMapINLINE().getSeaLevel() == 2) //medium likelihood
			iClusteringStrength = 100;
		else if (GC.getMapINLINE().getSeaLevel() == 3) { //low likelihood
			iClusteringStrength = 80;
		}
		else if (GC.getMapINLINE().getSeaLevel() == 4) { //very low likelihood
			iClusteringStrength = 40;
		}
		
		if (iClusteringValue < 100)
			iClusteringValue = std::max(10, 100 - (100-iClusteringValue)*iClusteringStrength/100);
		else if (iClusteringValue > 100)
			iClusteringValue = std::min(500, 100 + (iClusteringValue-100)*iClusteringStrength/100);
		iValue *= iClusteringValue;
		iValue /= 100;

		//Rhye RFCRAND - start
		//char buf[30];
		if (bDebugAll || bDebugSL) {
			//sprintf(buf, "Tot iClusteringValue: %d;", iClusteringValue);
			//GC.getGameINLINE().logMsg(buf);
		}
		//Rhye - end
	}
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10d Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	

	if (!bStartingLoc) { //continent
		if (pStartingPlot != NULL) {
			if (pPlot->area()->getID() == pStartingPlot->area()->getID()) {
				iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(0);
				iValue /= 100;
			}
			else {
				iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(0);
			...
			if (GET_PLAYER(getID()).getSettlersModifiersNorm(0) < 100 && getNumCities() >= settlersCoords[getID()][6]-1) {
				if (pPlot->area()->getID() == pStartingPlot->area()->getID()) {
					iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(0);
					iValue /= 100;
				}
				//else {
				//	iValue *= 200-GET_PLAYER(getID()).getSettlersModifiersNorm(0);
				//	iValue /= 100;
				//}
			}


			if (GC.getGameINLINE().getGameTurn() > 220)
			{
				/*if (GC.getMapINLINE().getSeaLevel() <= 1) {
					int IDSAmerica = -1;
					int IDNAmerica = -1;
					int IDCAmerica = -1;
					if (GET_PLAYER((PlayerTypes)INCA).isAlive())
						IDSAmerica = GET_PLAYER((PlayerTypes)INCA).getStartingPlot()->area()->getID();
					if (GET_PLAYER((PlayerTypes)AZTEC).isAlive())
						IDNAmerica = GET_PLAYER((PlayerTypes)AZTEC).getStartingPlot()->area()->getID();
					if (GET_PLAYER((PlayerTypes)MAYA).isAlive())
						IDCAmerica = GET_PLAYER((PlayerTypes)MAYA).getStartingPlot()->area()->getID();
					int currentArea = pPlot->area()->getID();

					
					if ((currentArea == IDNAmerica || currentArea == IDCAmerica || currentArea == IDSAmerica) && currentArea == pStartingPlot->area()->getID()) 
					{
						if (GET_PLAYER(getID()).getSettlersModifiersNorm(0) <= 60)
						{
							iValue *= 500;
							iValue /= 100;
						}
					...
					if (GC.getMapINLINE().plotINLINE(iX, iY)->isNewWorld() && !GC.getMapINLINE().plotINLINE(pStartingPlot->getX(), pStartingPlot->getY())->isNewWorld()) {
						if (GET_PLAYER(getID()).getSettlersModifiersNorm(0) <= 60)
						{
							iValue *= 600;
							iValue /= 100;
	...
	if (!bStartingLoc) {
		if (bIsCoastal) {
			iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(1);
			iValue /= 100;
		}
	}
	else {
		bool bNextToCoastal = false;
		bool bCloseToOcean = false;	
		for (int iLoopX = iX-3; iLoopX < iX+4; iLoopX++) { //2 and 3?
			for (int iLoopY = iY-3; iLoopY < iY+4; iLoopY++) {
				CvPlot* pLoopPlot = GC.getMapINLINE().plotINLINE(iLoopX, iLoopY);
				if (pLoopPlot != NULL) {
					if (!pPlot->isCoastalLand(GC.getMIN_WATER_SIZE_FOR_OCEAN()) && pLoopPlot->isCoastalLand(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
					//if (pPlot->isWater())
						bNextToCoastal = true;
					if (pLoopPlot->area()->getID() == GC.getMapINLINE().plotINLINE(0, 0)->area()->getID())
						bCloseToOcean = true;
				}
			}
		}

		int iCoords5 = settlersCoords[getID()][5];
		if (iCoords5 == 3 || iCoords5 == 5) {
			if (bIsCoastal) {
				iValue *= 200;
				iValue /= 100;
			}
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10e Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	if (pPlot->getTerrainType() == 2) { //desert
		iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(5);
		iValue /= 100;
	}
	else {
		iValue *= 200-GET_PLAYER(getID()).getSettlersModifiersNorm(5);
		iValue /= 100;
	}

	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10f Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	if (pPlot->isRiverSide()) {
		iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(2);
		iValue /= 100;
	}
	else {
		iValue *= 200-GET_PLAYER(getID()).getSettlersModifiersNorm(2);
		iValue /= 100;
	}

	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10g Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	if (bStartingLoc) {
		if (pPlot->getTerrainType() == 3) { //tundra, shouldn't start there
			iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(6);
			iValue /= 100;
		}
	}
	...
	int numPeaks = 0;
	int numPassable = 0;
	for (int iLoopX = iX-2; iLoopX < iX+3; iLoopX++) {
		for (int iLoopY = iY-2; iLoopY < iY+3; iLoopY++) {
			if (!(abs(iLoopX-iX) == 2 && abs(iLoopY-iY) == 2)) {
				CvPlot* pLoopPlot = GC.getMapINLINE().plotINLINE(iLoopX, iLoopY);
		//for (int iI = 0; iI < NUM_DIRECTION_TYPES; ++iI) {
		//	CvPlot* pLoopPlot = plotDirection(iX, iY, ((DirectionTypes)iI));
				if (pLoopPlot != NULL) {

					if (pLoopPlot->getFeatureType() == 1 || pLoopPlot->getFeatureType() == 6) //jungle or marsh
						numJungles++;
					if (pLoopPlot->isPeak())
						numPeaks++;
					if (canFound(iLoopX, iLoopY) || pLoopPlot->getTerrainType() == 2) //settleable or desert
						numPassable++;
				}
			}
		}
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10h Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
	...
	if (numJungles >= 2) {
		iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(4);
		iValue /= 100;
		if (numJungles >= 4) {
			iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(4);
			iValue /= 100;
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 10i Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end

	if (numPeaks >= 3) {
		iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(3);
		iValue /= 100;
		if (numPeaks >= 4) {
			iValue *= GET_PLAYER(getID()).getSettlersModifiersNorm(3);
			iValue /= 100;
	...
	//Rhye RFCRAND - start
	if (bDebugAll || bDebugSL) {
		sprintf(buf, "Test 11 Player %d (%d,%d): %d;", getID(), iX, iY, iValue);
		GC.getGameINLINE().logMsg(buf);
	}
	//Rhye - end
...
//Rhye RFCRAND - start
//int CvPlayer::AI_blindSettlersMap(int iX, int iY, bool bStartingLoc)
//{
//	int iValue = 100;
//	CvPlot* pPlot = GC.getMapINLINE().plotINLINE(iX, iY);
//
//	if (pPlot->getLatitude() <= settlersCoords[getID()][2]) {
//		iValue *= std::max(60,(100 - (settlersCoords[getID()][2]-pPlot->getLatitude())));
//		iValue /= 100;
//	}
//	else if (pPlot->getLatitude() >= settlersCoords[getID()][3]) {
//		iValue *= std::max(60,(100 - (settlersCoords[getID()][3]-pPlot->getLatitude())));
//		iValue /= 100;
//	}
//
//	if (!bStartingLoc) { //longitude
//		int iMaxExtension = settlersCoords[getID()][4]*GC.getMapINLINE().getGridWidthINLINE()/100;
//		if ((iX + GC.getMapINLINE().getGridWidthINLINE() >= getStartingPlot()->getX() + iMaxExtension + GC.getMapINLINE().getGridWidthINLINE())) {
//			iValue *= std::max(60, 100 - (iX - (getStartingPlot()->getX() + iMaxExtension)));
//			iValue /= 100;
//		}
//		else if ((iX + GC.getMapINLINE().getGridWidthINLINE() <= getStartingPlot()->getX() - iMaxExtension + GC.getMapINLINE().getGridWidthINLINE()) ) {
//			iValue *= std::max(60, 100 - ((getStartingPlot()->getX() + iMaxExtension) - iX));
//			iValue /= 100;
//		}
//	}
//
//	if (!bStartingLoc) { //continent
//		if (pPlot->area()->getID() == getStartingPlot()->area()->getID()) {
//			iValue *= settlersModifiers[getID()][0];
//			iValue /= 100;
//		}
//	}
//
...
//
//	if (pPlot->getTerrainType() == 2) { //desert
//		iValue *= settlersModifiers[getID()][5];
//		iValue /= 100;
//	}
//	if (pPlot->isCoastalLand(GC.getMIN_WATER_SIZE_FOR_OCEAN())) {
//		iValue *= settlersModifiers[getID()][1];
//		iValue /= 100;
//	}
//	if (pPlot->isRiverSide()) {
//		iValue *= settlersModifiers[getID()][2];
//		iValue /= 100;
//	}
//	int numJungles = 0;
//	int numPeaks = 0;
//	for (int iLoopX = iX-1; iLoopX < iX+2; iLoopX++) {
//		for (int iLoopY = iY-1; iLoopY < iY+2; iLoopY++) {
//			CvPlot* pLoopPlot = GC.getMapINLINE().plotINLINE(iLoopX, iLoopY);
...
//	}
//	if (numJungles >= 3) {
//			iValue *= settlersModifiers[getID()][4];
//			iValue /= 100;
//			if (numJungles >= 5) {
//				iValue *= settlersModifiers[getID()][4];
//				iValue /= 100;
...
//		}
//	if (numPeaks >= 3) {
//			iValue *= settlersModifiers[getID()][3];
//			iValue /= 100;
//			if (numPeaks >= 5) {
//				iValue *= settlersModifiers[getID()][3];
//				iValue /= 100;
...
bool CvPlayerAI::AI_isFinancialTrouble() const
{
	//if (getCommercePercent(COMMERCE_GOLD) > 50)
	{
		...
		if (iGameTurn == GET_PLAYER((PlayerTypes)getID()).getBirthTurn() || iGameTurn == GET_PLAYER((PlayerTypes)getID()).getBirthTurn() + 1 || iGameTurn == GET_PLAYER((PlayerTypes)getID()).getBirthTurn() + 2) //RFCRAND
			return false;
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			for (iJ = 0; iJ < NUM_CONTACT_TYPES; iJ++)
			{
				if (AI_getContactTimer(((PlayerTypes)iI), ((ContactTypes)iJ)) > 0)
				{
					...
					//Rhye - start RFCRAND (workaround for contacts bug)
					if (m_iAdditionalContactTimer > 0)
							m_iAdditionalContactTimer -= 1;
					//Rhye - end 
				}
			}
		}
						...
						if (canContact((PlayerTypes)iI) && AI_isWillingToTalk((PlayerTypes)iI))
						{
							...
							if (GET_PLAYER((PlayerTypes)iI).getTeam() != getTeam() && !(GET_TEAM(getTeam()).isHuman()) && (GET_PLAYER((PlayerTypes)iI).isHuman() || !(GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHuman())))
							{
								...
								if (!(GET_TEAM(getTeam()).isAtWar(GET_PLAYER((PlayerTypes)iI).getTeam())))
								{
									...
									{
										//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_PERMANENT_ALLIANCE) == 0)//Rhye - RFCRAND (workaround for contacts bug)
										if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_PERMANENT_ALLIANCE) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
										{
											bool bOffered = false;
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()) && !GET_TEAM(getTeam()).isVassal(GET_PLAYER((PlayerTypes)iI).getTeam()))
									{
										if (getStateReligion() != NO_RELIGION)
										{
											...
											{
												//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_RELIGION_PRESSURE) == 0) //Rhye - RFCRAND (workaround for contacts bug)
												if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_RELIGION_PRESSURE) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
												{
										...
										if (eFavoriteCivic != NO_CIVIC)
										{
											if (isCivic(eFavoriteCivic))
											{
												if (GET_PLAYER((PlayerTypes)iI).canDoCivics(eFavoriteCivic) && !(GET_PLAYER((PlayerTypes)iI).isCivic(eFavoriteCivic)))
												{
													...
													{
														//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_CIVIC_PRESSURE) == 0) //Rhye - RFCRAND (workaround for contacts bug)
														if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_CIVIC_PRESSURE) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
														{
									...
									//Rhye - leave commented: they cause crash
									//char buf[30];
									//sprintf(buf, "GET_PLAYER((PlayerTypes)iI).isHuman(): %d", GET_PLAYER((PlayerTypes)iI).isHuman());
									//GC.getGameINLINE().logMsg(buf);
									
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()))
									{
										//Rhye - leave commented: they cause crash
										//sprintf(buf, "iI, getID(): %d,%d", iI, getID());
										//GC.getGameINLINE().logMsg(buf);

										if ((AI_getMemoryCount(((PlayerTypes)iI), MEMORY_DECLARED_WAR) == 0) && (AI_getMemoryCount(((PlayerTypes)iI), MEMORY_HIRED_WAR_ALLY) == 0))
										{
											//Rhye - leave commented: they cause crash
											//sprintf(buf, "AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR): %d", AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR));
											//GC.getGameINLINE().logMsg(buf);

											//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR) == 0) //Rhye - RFCRAND (workaround for contacts bug)
											if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
											{
												...
												{
													//Rhye - leave commented: they cause crash
													//sprintf(buf, "GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_JOIN_WAR): %d", GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_JOIN_WAR));
													//GC.getGameINLINE().logMsg(buf);
													...
													if (eBestTeam != NO_TEAM)
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															...
															//Rhye - leave commented: they cause crash
															//sprintf(buf, "CONTACT_JOIN_WAR, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_JOIN_WAR): %d, %d", (int)CONTACT_JOIN_WAR, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_JOIN_WAR));
															//GC.getGameINLINE().logMsg(buf);
															//sprintf(buf, "AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR): %d", AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR));
															//GC.getGameINLINE().logMsg(buf);
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()) && !GET_TEAM(getTeam()).isVassal(GET_PLAYER((PlayerTypes)iI).getTeam()))
									{
										//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_STOP_TRADING) == 0)//Rhye - RFCRAND (workaround for contacts bug)
										if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_STOP_TRADING) == 0  && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
										{
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()))
									{
										if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).getAssets() < (GET_TEAM(getTeam()).getAssets() / 2))
										{
											...
											{
												//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_GIVE_HELP) == 0)//Rhye - RFCRAND (workaround for contacts bug)
												if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_GIVE_HELP) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0) )
												{
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()))
									{
										if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).getAssets() > (GET_TEAM(getTeam()).getAssets() / 2))
										{
											//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_ASK_FOR_HELP) == 0)//Rhye - RFCRAND (workaround for contacts bug)
											if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_ASK_FOR_HELP) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
											{
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()))
									{
										if (GET_TEAM(getTeam()).canDeclareWar(GET_PLAYER((PlayerTypes)iI).getTeam()) && !GET_TEAM(getTeam()).AI_isSneakAttackPreparing(GET_PLAYER((PlayerTypes)iI).getTeam()))
										{
											if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).getDefensivePower() < GET_TEAM(getTeam()).getPower(true))
											{
												...
												{
													//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE) == 0)//Rhye - RFCRAND (workaround for contacts bug)
													if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0) )
													{
									...
									if (GET_TEAM(getTeam()).getLeaderID() == getID())
									{
										//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_OPEN_BORDERS) == 0)//Rhye - RFCRAND (workaround for contacts bug)
										if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_OPEN_BORDERS) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0) )
										{
									...
									if (GET_TEAM(getTeam()).getLeaderID() == getID())
									{
										//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEFENSIVE_PACT) == 0)//Rhye - RFCRAND (workaround for contacts bug)
										if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEFENSIVE_PACT) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0) )
										{
									...
									if (!(GET_PLAYER((PlayerTypes)iI).isHuman()) || (GET_TEAM(getTeam()).getLeaderID() == getID()))
									{
										//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_TECH) == 0)//Rhye - RFCRAND (workaround for contacts bug)
										if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_TECH) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0) )
										{
									...
									//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_BONUS) == 0) //Rhye - RFCRAND (workaround for contacts bug)
									if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_BONUS) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0) )
									{
									...
									//if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_MAP) == 0) //Rhye - RFCRAND (workaround for contacts bug)
									if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_TRADE_MAP) == 0 && !(GET_PLAYER((PlayerTypes)iI).isHuman() && m_iAdditionalContactTimer > 0))
									{
...
int CvPlayerAI::AI_browseStep(int iMinFoundValueThreshold, int iBestFoundValue, CvPlot* pBestFoundPlot, int iModifier)
{
	...
	for (iJ = 0; iJ < EARTH_Y; iJ++)
	{
		...
		//if (settlersMaps[getID()][EARTH_Y -1 -iJ][iI] == iModifier) //Rhye - RFCRAND
