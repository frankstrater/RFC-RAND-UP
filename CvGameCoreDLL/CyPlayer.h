...
public:
	...
	bool isMinorCiv();
	void setMinorCiv(bool bMinorNationCiv); //Rhye RFCRAND
	bool isAlive();
	bool isEverAlive();
	bool isExtendedGame();
	...
	int AI_blindSettlersMap(int iX, int iY, bool bStartingLoc); //Rhye RFCRAND
	...
	int getSettlersCoords(int index); //Rhye
	int getSettlersModifiers(int index); //Rhye
	int getSettlersModifiersNorm(int index); //Rhye
	
	//void setFlag(CvWString s); //Rhye
	void setLeader(int i); //Rhye
	int /*LeaderHeadTypes*/ getLeader(); //Rhye
	int getBirthTurn(); //Rhye RFCRAND
	void setBirthTurn(int i); //Rhye RFCRAND
