...
void CvGame::init(HandicapTypes eHandicap)
{
	...
	//Rhye - RFCRAND start
	if (GC.getMapINLINE().getSeaLevel() == 4) //very low
		setVictoryValid((VictoryTypes)7, false); //historical
...
void CvGame::setInitialItems()
{
	...
	/*for (int i = 0; i < BARBARIAN; i++) {
		sprintf(buf, "Player %d slot status: %d", i, GC.getInitCore().getSlotStatus( (PlayerTypes)i ));
		GC.getGameINLINE().logMsg(buf);
	}*/
	//the player 0 bug
	int iHumanCiv = GC.getInitCore().getCiv((PlayerTypes)0);
	int iHumanLeader = GC.getInitCore().getLeader((PlayerTypes)0);
	bool bHuman = false;
	for (int iLoopPlayer = 0; iLoopPlayer < BARBARIAN; iLoopPlayer++) {
		bHuman = false;
		int newLeader = assignedEarlyLeader[iLoopPlayer];
		if (iHumanCiv == (CivilizationTypes)assignedCiv[iLoopPlayer]) {
			bHuman = true;
			newLeader = iHumanLeader;
		}
		
		changePlayer( iLoopPlayer, assignedCiv[iLoopPlayer], newLeader, iLoopPlayer, bHuman, true );
		
		//GET_PLAYER((PlayerTypes)iLoopPlayer).setTeam((TeamTypes)iLoopPlayer); //for replacing custom game settings which may mess up team numbers - but doesn't work
		if (bHuman) {
			setActivePlayer((PlayerTypes)iLoopPlayer, false);
			//GET_PLAYER((PlayerTypes)iLoopPlayer).setLeader(iHumanLeader);
			//GET_PLAYER((PlayerTypes)iLoopPlayer).setHandicapType(GET_PLAYER((PlayerTypes)0).getHandicapType());
			GET_PLAYER((PlayerTypes)iLoopPlayer).setHandicapType(getHandicapType());
		}			
		if (iLoopPlayer == 0) //otherwise the leader name is replaced by player name
			GC.getInitCore().setLeaderName((PlayerTypes)EGYPT, GC.getLeaderHeadInfo((LeaderHeadTypes)assignedEarlyLeader[EGYPT]).getDescription(0));
	}
	if (!GET_PLAYER((PlayerTypes)0).isHuman())
		GET_PLAYER((PlayerTypes)0).setHandicapType((HandicapTypes)2);  //monarch
	/*for (int i = 0; i < BARBARIAN; i++) {
		sprintf(buf, "Player %d slot status: %d", i, GC.getInitCore().getSlotStatus( (PlayerTypes)i ));
		GC.getGameINLINE().logMsg(buf);
	}*/
	...
	//Rhye - RFCRAND
	//setupBirthTurnModifiers(); //called from RandomRFC.py



	initFreeState();
	//assignStartingPlots(); //Rhye RFCRAND
	//normalizeStartingPlots(); //Rhye RFCRAND
	//initFreeUnits(); //Rhye RFCRAND
...
void CvGame::initDiplomacy()
{
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		if (GET_TEAM((TeamTypes)iI).isBarbarian() || GET_TEAM((TeamTypes)iI).isMinorCiv())
		{
			...
			else if (iI == CELTIA && GET_PLAYER((PlayerTypes)EGYPT).isPlayable()) { //late start condition
...
void CvGame::update()
{
	...
	if (!gDLL->GetWorldBuilderMode() || isInAdvancedStart())
	{
		...
		//RFCRAND
		int iBirthTurn = GET_PLAYER(getActivePlayer()).getBirthTurn();
		if (iBirthTurn != 0) {
			if (getGameTurn() == 0)
				setAIAutoPlay(1);
			else if (getGameTurn() <= iBirthTurn)
				setAIAutoPlayCatapult(1);  
		}
		//Rhye - end
	}
...
//Rhye - RFCRAND start
void CvGame::setupBirthTurnModifiers() {
	int likeness = GC.getMapINLINE().getSeaLevel();
	if (likeness == 0) {
		for (int iCiv=0;iCiv<NUM_MAJOR_PLAYERS;iCiv++)
			GET_PLAYER((PlayerTypes)iCiv).setBirthTurn(startingTurn[iCiv]);
		return;
	}
	else {
		if (likeness <= 2) {
			int iMaxValue = 8; //medium likeliness
            if (likeness == 4) //low
				iMaxValue = 20;
            for (int iCiv=0;iCiv<NUM_MAJOR_PLAYERS;iCiv++)
				if (startingTurn[iCiv] == 0) 
					GET_PLAYER((PlayerTypes)iCiv).setBirthTurn(0);
				else {
					bool bSameTurn = true; 
					while (bSameTurn) { //now make sure that no civs spawn in the same turn and cause a double "new civ" popup
						bSameTurn = false;
						GET_PLAYER((PlayerTypes)iCiv).setBirthTurn(startingTurn[iCiv] + getSorenRandNum(iMaxValue*2+1, "BirthTurnModifier") - (iMaxValue/2));
						for (int jCiv=0;jCiv<NUM_MAJOR_PLAYERS;jCiv++)
							if (iCiv != jCiv)
								if (GET_PLAYER((PlayerTypes)iCiv).getBirthTurn() == GET_PLAYER((PlayerTypes)jCiv).getBirthTurn())
									bSameTurn = true;
					}
				}
			}

		if (likeness >= 3) { //very low
			...
			for (int iCiv=0;iCiv<NUM_MAJOR_PLAYERS;iCiv++) {
				...
				CyArgsList argsList;
				argsList.add(iCiv); 
				gDLL->getPythonIFace()->callFunction(PYScreensModule, "getInThisGame", argsList.makeFunctionArgs(), &result);
				int iResult = (int)result;
				if (iResult == 1)
					GET_PLAYER((PlayerTypes)iCiv).setBirthTurn(0);
			}	
			int iNumStarters = 4;
			int iNumPlayersInThisGame = NUM_MAJOR_PLAYERS;
			if (GC.getMapINLINE().getWorldSize() == 0) {
				iNumStarters = 3;
				iNumPlayersInThisGame = 15;
			}
			else if (GC.getMapINLINE().getWorldSize() == 1) {
				iNumStarters = 3;
				iNumPlayersInThisGame = 21;
			}
			int iCounter = 0;
			

			while (iCounter < iNumPlayersInThisGame-iNumStarters) {
				int iChosenCiv = getSorenRandNum(NUM_MAJOR_PLAYERS, "");
				if (likeness == 3) { //try again to leave out an early starter 
					if (iChosenCiv <= GREECE) {
						if (getSorenRandNum(3, "roll") <= 1) {
							iChosenCiv = getSorenRandNum(NUM_MAJOR_PLAYERS, "");
						}
					}
					else if (iChosenCiv <= MAYA) {
						if (getSorenRandNum(3, "roll") == 0) {
							iChosenCiv = getSorenRandNum(NUM_MAJOR_PLAYERS, "");
						}
					}
				}
				if (GET_PLAYER((PlayerTypes)iChosenCiv).getBirthTurn() == 0) {
					bool bSameTurn = true;
					int iChosenTurn = 0;
					if (isOption(GAMEOPTION_3000BC) && iChosenCiv == getActivePlayer())
							continue;
					while (bSameTurn) { //now make sure that no civs spawn in the same turn and cause a double "new civ" popup
						bSameTurn = false;
						iChosenTurn = getSorenRandNum(355, "") + 5;
						if (likeness == 3) { //in the range
							if (iChosenTurn < startingRange[iChosenCiv][0] || iChosenTurn > startingRange[iChosenCiv][1]) {
									iChosenTurn = getSorenRandNum(355, "") + 5;
							}	
							if (iChosenTurn < startingRange[iChosenCiv][0] || iChosenTurn > startingRange[iChosenCiv][1]) {
									iChosenTurn = getSorenRandNum(355, "") + 5;
							...
							if (iChosenTurn < startingRange[iChosenCiv][0] || iChosenTurn > startingRange[iChosenCiv][1]) {
								if (iChosenCiv == getActivePlayer() || getSorenRandNum(2, "roll") == 0) {
									iChosenTurn = getSorenRandNum(355, "") + 5;
								}
							}
						}
						if (iChosenCiv == EGYPT || iChosenCiv == GREECE || iChosenCiv == CARTHAGE || iChosenCiv == MAYA) //critical UP
							if (iChosenTurn > 250)
								iChosenTurn /= 2;

						//if (iChosenTurn > 250) //less likely to pick a late start
						//	iChosenTurn = getSorenRandNum(355, "") + 5;
						//if (iChosenCiv == getActivePlayer()) {
						//	iChosenTurn = getSorenRandNum(190, "") + 10; //max Medieval for human player
							//debug
							//if (iChosenCiv == EGYPT)
							//		iChosenCiv = getSorenRandNum(NUM_MAJOR_PLAYERS, "");
						//}	
						for (int jCiv=0;jCiv<NUM_MAJOR_PLAYERS;jCiv++)
							if (iChosenCiv != jCiv)
								if (iChosenTurn == GET_PLAYER((PlayerTypes)jCiv).getBirthTurn()) {
									bSameTurn = true;
									break;
								}
					}
					GET_PLAYER((PlayerTypes)iChosenCiv).setBirthTurn(iChosenTurn);
					iCounter++;

					//sprintf(buf, "iCounter: %d; iChosenCiv: %d; startingTurn[iChosenCiv]: %d;", iCounter,iChosenCiv,startingTurn[iChosenCiv]);
					//GC.getGameINLINE().logMsg(buf);
				}
			}
		}
			...
			if (pPlot->isUnit()) {
				...
				for (int iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isHuman())
					{
						...
						GC.getMapINLINE().plotINLINE(2, GC.getMapINLINE().getGridHeightINLINE()-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(1, GC.getMapINLINE().getGridHeightINLINE()-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(0, GC.getMapINLINE().getGridHeightINLINE()-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(GC.getMapINLINE().getGridWidthINLINE()-1, GC.getMapINLINE().getGridHeightINLINE()-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(GC.getMapINLINE().getGridWidthINLINE()-2, GC.getMapINLINE().getGridHeightINLINE()-1)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(2, GC.getMapINLINE().getGridHeightINLINE()-2)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(1, GC.getMapINLINE().getGridHeightINLINE()-2)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(0, GC.getMapINLINE().getGridHeightINLINE()-2)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(GC.getMapINLINE().getGridWidthINLINE()-1, GC.getMapINLINE().getGridHeightINLINE()-2)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
						GC.getMapINLINE().plotINLINE(GC.getMapINLINE().getGridWidthINLINE()-2, GC.getMapINLINE().getGridHeightINLINE()-2)->setRevealed(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false, NO_TEAM, true);
					}
				}		
			}
			else {
