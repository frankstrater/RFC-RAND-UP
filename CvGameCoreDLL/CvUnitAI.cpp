...
bool CvUnitAI::AI_foundRange(int iRange, bool bFollow)
{
	...
	//Rhye - RFCRAND
	//int EARTH_X = GC.getMapINLINE().getGridWidthINLINE(); //Rhye
	//int EARTH_Y = GC.getMapINLINE().getGridHeightINLINE(); //Rhye
		...
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (AI_plotValid(pLoopPlot) && (pLoopPlot != plot() || GET_PLAYER(getOwnerINLINE()).AI_getPlotDanger(pLoopPlot, 1) <= pLoopPlot->plotCount(PUF_canDefend, -1, -1, getOwnerINLINE())))
				{
					if (canFound(pLoopPlot))
					{
						//if (getOwnerINLINE() > NUM_MAJOR_PLAYERS || settlersMaps[getOwnerINLINE()][EARTH_Y -1 - (iDY+getY_INLINE())][(iDX+getX_INLINE())] >= 60) //Rhye
						//{ //Rhye RFCRAND
