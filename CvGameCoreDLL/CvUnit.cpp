...
bool CvUnit::canMoveInto(const CvPlot* pPlot, bool bAttack, bool bDeclareWar, bool bIgnoreLoad) const
{
	...
	//Rhye - start
	if (getUnitType() == 89) //workboat
	{
		if (NO_PLAYER == pPlot->getOwnerINLINE()) {
			//if (AI_getUnitAIType() == UNITAI_EXPLORE_SEA) {
				//AI_setUnitAIType(UNITAI_WORKER_SEA);
				//GET_PLAYER(getOwnerINLINE()).initUnit(getUnitType(), getX_INLINE(), getY_INLINE(), UNITAI_WORKER_SEA);
				//kill(false);
				//}
				CvPlot* pLoopPlot;
				bool outerSea = true;
				...
				{					
					...
					if (pLoopPlot != NULL)
					{
						if (NO_PLAYER != pLoopPlot->getOwnerINLINE())
							outerSea = false;	
					}
				}
				if (outerSea)
...
bool CvUnit::canUpgrade(UnitTypes eUnit, bool bTestVisible) const
{
	...
	//Rhye - start mercenaries fix
	if (isHasPromotion((PromotionTypes)54)) //Mercenary
	{
		return false;
	}
	//Rhye - end
...
int CvUnit::workRate(bool bMax) const
{
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (getOwnerINLINE()<NUM_MAJOR_PLAYERS) {
			iRate -= (iRate*(startingTurn[getOwnerINLINE()] - GET_PLAYER((PlayerTypes)getOwnerINLINE()).getBirthTurn())/580);
		}
	}
