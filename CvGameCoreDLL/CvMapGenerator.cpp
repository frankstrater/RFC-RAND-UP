...
bool CvMapGenerator::canPlaceBonusAt(BonusTypes eBonus, int iX, int iY, bool bIgnoreLatitude)
{
	...
	//Rhye RFCRAND
	//char buf[30];
	//sprintf(buf, "CANHAVE1 eBonus: %d;iX: %d;iY: %d;", eBonus,iX, iY);
	...
	//Rhye RFCRAND
	//sprintf(buf, "CANHAVE2 eBonus: %d;iX: %d;iY: %d;", eBonus,iX, iY);
	//GC.getGameINLINE().logMsg(buf);
	//Rhye - end debug

	int iCountAdjacent = 0; //Rhye (RFCRAND)
	...
	//Rhye (RFCRAND)
	if (iCountAdjacent >= 2) //groups of up to two 
		return false;

	//Rhye - start RFCRAND
	//sprintf(buf, "CANHAVE3 eBonus: %d;iX: %d;iY: %d;", eBonus,iX, iY);
	//GC.getGameINLINE().logMsg(buf);
	//Rhye - end debug
	...
	//Rhye RFCRAND
	//sprintf(buf, "CANHAVE4 eBonus: %d;iX: %d;iY: %d;", eBonus,iX, iY);
	//GC.getGameINLINE().logMsg(buf);
	//Rhye - end debug
	...
	//Rhye RFCRAND
	//sprintf(buf, "CANHAVE5 eBonus: %d;iX: %d;iY: %d;", eBonus,iX, iY);
	//GC.getGameINLINE().logMsg(buf);
	//Rhye - end debug
	...
	//Rhye RFCRAND
	//sprintf(buf, "TRUE eBonus: %d;iX: %d;iY: %d;", eBonus,iX, iY);
	//GC.getGameINLINE().logMsg(buf);
	//Rhye - end debug
		...
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			if (!(pLoopPlot->isWater()))
			{
				if (((iPass == 0) && (pLoopPlot->isHills() || pLoopPlot->isPeak())) ||
					  ((iPass == 1) && !(pLoopPlot->isCoastalLand()) && (GC.getGameINLINE().getMapRandNum(4, "addRivers") == 0)) ||    //8 Rhye RFCRAND
		...
		for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			{		
				if (canPlaceBonusAt(eBonusType, pPlot->getX_INLINE(), pPlot->getY_INLINE(), bIgnoreLatitude))
				{
					...
					/*char buf[30];
					if (eBonusType == 29) {
						sprintf(buf, "UNIQUE; eBonusType: %d", eBonusType);
					GC.getGameINLINE().logMsg(buf); }*/
					//Rhye - end debug
...
void CvMapGenerator::addNonUniqueBonusType(BonusTypes eBonusType)
{
	...
	/*char buf[30];
	if (eBonusType == 29) {
	sprintf(buf, "NON UNIQUE; eBonusType: %d", eBonusType);
	GC.getGameINLINE().logMsg(buf); }*/
	//Rhye - end debug
...
int CvMapGenerator::calculateNumBonusesToAdd(BonusTypes eBonusType)
{
	...
	//Rhye RFCRAND
	/*char buf[30];
	sprintf(buf, "eBonusType: %d;iBonusCount: %d;", eBonusType,iBonusCount);
	GC.getGameINLINE().logMsg(buf);*/
	//Rhye - end debug
