...
CvPlayer::CvPlayer()
{
	m_aisettlersModifiersNorm = new int[7]; //Rhye RFCRAND
...
void CvPlayer::reset(PlayerTypes eID, bool bConstructorCall)
{
	...
	m_iAdditionalContactTimer = 0; //Rhye - RFCRAND (workaround for contacts bug)
	m_iBirthTurn = -1; //Rhye - RFCRAND
	m_uiStartTime = 0;
	...
	for (iI = 0; iI < 7; iI++)
	{
		m_aisettlersModifiersNorm[iI] = settlersModifiers[getID()][iI];
		if (GC.getMapINLINE().getSeaLevel() >= 3) { //low likelihood
			int iDivide = 2;
			if (GC.getMapINLINE().getSeaLevel() == 4) { //very low likelihood
				iDivide = 4;
			}
			m_aisettlersModifiersNorm[iI] = (settlersModifiers[getID()][iI] + (iDivide-1)*100) /iDivide;
		}
	}
	//Rhye - end
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			...
			if (GC.getBuildingInfo(eLoopBuilding).getFreeStartEra() != NO_ERA) //Rhye
			{
				...
				//RFCRAND
				if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
					if (getID()<NUM_MAJOR_PLAYERS)
						startingEra = int(getBirthTurn()/90);
					switch (getID())
					{
						case SPAIN:
						case FRANCE:
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	if (GC.getBuildingInfo(eBuilding).getMaxStartEra() != NO_ERA)
	{
		...
		if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
			if (getID()<NUM_MAJOR_PLAYERS)
				startingEra = int(m_iBirthTurn/90); //why can't I use getBirthTurn()??
		}
...
int CvPlayer::getProductionNeeded(UnitTypes eUnit) const
{
	...
	//Rhye RFCRAND - bugfix (cheap settler). 
	if (iProductionNeeded <= 50 && (int)eUnit == 4) {
		//iProductionNeeded = 130; 
		if (getCurrentEra() == 0) { //ancient
			iProductionNeeded = 130; 
		}
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (getID()<NUM_MAJOR_PLAYERS)
			iProductionNeeded += (iProductionNeeded*(startingTurn[getID()] - GET_PLAYER((PlayerTypes)getID()).getBirthTurn())/1300);
	}
...
int CvPlayer::getProductionNeeded(BuildingTypes eBuilding) const
{
	...
	if (isWorldWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eBuilding).getBuildingClassType()))) 
	{
		switch (getID())
		{
			...
			case BABYLONIA:
				...
				break;
			case GREECE:
				iProductionNeeded *= 85; //RFCRAND - 80 in RFC
				iProductionNeeded /= 100;
				break;
			case PERSIA:
				iProductionNeeded *= 85;
	...
	else
	{
		...
		if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
			if (getID()<NUM_MAJOR_PLAYERS)
				iProductionNeeded += (iProductionNeeded*(startingTurn[getID()] - GET_PLAYER((PlayerTypes)getID()).getBirthTurn())/900);
		}
		switch (getID())
		{
			case EGYPT:
...
int CvPlayer::calculateInflationRate() const
{
	...
	switch (getID())
	{
		...
		case NETHERLANDS:				
			...
			break;
		case MALI:
			iRate *= 120; //115 RFCRAND //130 before No tech brokering   //80 before removal of Financian trait //higher to counter its UP
			iRate /= 100;
			break;
		case TURKEY:
			iRate *= 79;
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (getID()<NUM_MAJOR_PLAYERS)
			iRate += (iRate*(startingTurn[getID()] - GET_PLAYER((PlayerTypes)getID()).getBirthTurn())/500);
	}
...
bool CvPlayer::canDoCivics(CivicTypes eCivic) const
{
	...
	if (getID() == EGYPT) {
		...
		if (GET_PLAYER((PlayerTypes)getID()).getBirthTurn() >= 50) //Bronze: already has Bronze Working
	  /*    if (eCivic == 12) //serfdom
				return true;
		if (GET_PLAYER((PlayerTypes)getID()).getBirthTurn() >= 170) //EarlyMedieval: already has Feudalism   */
			if (eCivic == 7) //burocracy
				return true;
		if (GET_PLAYER((PlayerTypes)getID()).getBirthTurn() >= 130) //Classical: already has Monarchy
			if (eCivic == 2) //representation
				return true;
	}
	//Rhye - end UP
...
int CvPlayer::greatPeopleThreshold(bool bMilitary) const
{
	...
	switch (getID())
	{
		...
		case BABYLONIA:
			...
			break;
		case GREECE:
			result = (iThreshold*115/100); //RFCRAND - 110 in RFC
			break;
		case PERSIA:
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (getID()<NUM_MAJOR_PLAYERS)
			result += (result*(startingTurn[getID()] - GET_PLAYER((PlayerTypes)getID()).getBirthTurn())/420);
	}
...
//Rhye RFCRAND - start
void CvPlayer::setMinorCiv(bool bMinorNationCiv)
{
	GC.getInitCore().setMinorNationCiv(getID(), bMinorNationCiv);
}
//Rhye - end
...
void CvPlayer::setCurrentEra(EraTypes eNewValue)
{
	...
	if (getCurrentEra() != eNewValue)
	{
		...
		if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
			if (getID()<NUM_MAJOR_PLAYERS)
				startEra = getCurrentEra(); //int(getBirthTurn()/88);
		}
...
//}
int CvPlayer::getSettlersCoords(int index)
{
	return settlersCoords[getID()][index];
}
int CvPlayer::getSettlersModifiers(int index)
{
	return settlersModifiers[getID()][index];
}
int CvPlayer::getSettlersModifiersNorm(int index)
{
	return m_aisettlersModifiersNorm[index];
}
...
void CvPlayer::read(FDataStreamBase* pStream)
{
	...
	pStream->Read(7, m_aisettlersModifiersNorm); //Rhye RFCRAND
	...
	pStream->Read(&m_iAdditionalContactTimer); //Rhye - RFCRAND (workaround for contacts bug)
	pStream->Read(&m_iBirthTurn); //Rhye - RFCRAND
}
...
void CvPlayer::write(FDataStreamBase* pStream)
{
	...
	pStream->Write(7, m_aisettlersModifiersNorm); //Rhye RFCRAND
	...
	pStream->Write(m_iAdditionalContactTimer); //Rhye - RFCRAND (workaround for contacts bug)
	pStream->Write(m_iBirthTurn); //Rhye - RFCRAND
}
...
int CvPlayer::getGrowthThreshold(int iPopulation) const
{
	...
	switch (getID())
	{
		...
		case INDIA:
			...
			break;
		case CHINA:
			iThreshold *= 133; //132 RFCRAND 
			iThreshold /= 100;
			break;
		case BABYLONIA:
			iThreshold *= 140; //130 RFCRAND
			iThreshold /= 100;
			break;
		case GREECE:
			iThreshold *= 138; //130 RFCRAND
			iThreshold /= 100;
			break;
		case PERSIA:
			iThreshold *= 133; //130 RFCRAND
			iThreshold /= 100;		
			break;
		case CARTHAGE:
			iThreshold *= 120;
			...
			break;
		case ROME:
			iThreshold *= 123; //120 RFCRAND
			iThreshold /= 100;
			break;
		case JAPAN:
			iThreshold *= 111; //112 RFCRAND
			iThreshold /= 100;
			break;
		case ETHIOPIA:
			iThreshold *= 105; //100 RFCRAND
			iThreshold /= 100;
			break;
		case MAYA:
			iThreshold *= 113; //110 RFCRAND
			iThreshold /= 100;
			break;
		case VIKING:
			iThreshold *= 80;
		...
		case RUSSIA:				
			...
			break;
		case NETHERLANDS:
			iThreshold *= 73; //73 RFCRAND  //Amsterdam is a powerhouse anyway
			iThreshold /= 100;
			break;
		case MALI:
			iThreshold *= 83; //78 RFCRAND
			iThreshold /= 100;
			break;
		case TURKEY:
			iThreshold *= 69;
			...
			break;
		case PORTUGAL:
			iThreshold *= 72; //73 RFCRAND
			iThreshold /= 100;
			break;
		case INCA:
			iThreshold *= 73; //69 RFCRAND
			iThreshold /= 100;
			break;
		case MONGOLIA:
			iThreshold *= 77; //74 RFCRAND
			iThreshold /= 100;
			break;
		case AZTEC:
			iThreshold *= 68;
	...
	if (GC.getMapINLINE().getSeaLevel() >= 3) { //very low
		if (getID()<NUM_MAJOR_PLAYERS)
			iThreshold += (iThreshold*(startingTurn[getID()] - GET_PLAYER((PlayerTypes)getID()).getBirthTurn())/420);
	}
	//Rhye - end
...
int CvPlayer::AI_blindSettlersMap(int iX, int iY, bool bStartingLoc)
{
	...
	if (pPlot->getLatitude() <= settlersCoords[getID()][2]) {
		iValue *= std::max(40,(100 - (settlersCoords[getID()][2]-pPlot->getLatitude())));
		iValue /= 100;
	}
	else if (pPlot->getLatitude() >= settlersCoords[getID()][3]) {
		iValue *= std::max(40,(100 - (settlersCoords[getID()][3]-pPlot->getLatitude())));
		iValue /= 100;
	}
	else {
		iValue *= 120;
...
int CvPlayer::getBirthTurn() {
	return m_iBirthTurn;
}; 

void CvPlayer::setBirthTurn(int iYear) {
	m_iBirthTurn = iYear;
};
