...
int CvTeam::getResearchCost(TechTypes eTech) const
{
	...
	if (getID() < NUM_MAJOR_PLAYERS) {		
		...
		if (GC.getGameINLINE().getGameTurn() <= GET_PLAYER((PlayerTypes)getID()).getBirthTurn() + iTurnModifier) { //RFCRAND
			
			iCost *= 100 - ((GET_PLAYER((PlayerTypes)getID()).getBirthTurn() + iTurnModifier) - GC.getGameINLINE().getGameTurn()); //RFCRAND
			iCost /= 100;
		}
	}
	//Rhye - end
