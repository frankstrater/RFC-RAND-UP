...
DenialTypes CvTeamAI::AI_techTrade(TechTypes eTech, TeamTypes eTeam) const
{
	...
	if (GC.getGameINLINE().getGameTurn() <= GET_PLAYER((PlayerTypes)getID()).getBirthTurn()+2) //RFCRAND
	{
		return DENIAL_TECH_MONOPOLY;
	}
	if (GC.getGameINLINE().getGameTurn() <= GET_PLAYER((PlayerTypes)eTeam).getBirthTurn()+2) //RFCRAND
	{
...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	if (GC.getGameINLINE().getGameTurn() <= GET_PLAYER((PlayerTypes)getID()).getBirthTurn()+5) //RFCRAND
	{
		iMasterPower /= 2;
	}
	//Rhye - end
...
int CvTeamAI::AI_defensivePactTradeVal(TeamTypes eTeam) const
{
	...
	else if (GC.getGameINLINE().getGameTurn() > 400) { //the last 100 turns, starting from 1900
		modifier = 160;
	}	
...
DenialTypes CvTeamAI::AI_defensivePactTrade(TeamTypes eTeam) const
{
	...
	//now the cap //RFCRAND: scale here
	int iCap = 3;
