    ...
    if (bMaybeWaterArea)
	{
		...
		if (NULL != pWaterArea)
		{
    			...
    			|| (iWaterPercent > 60 && GC.getGameINLINE().getSorenRandNum(4, "AI Train Early Sea Explore or Settler") == 0))
			{
				...
				{
					//if (!GET_TEAM((TeamTypes)getOwnerINLINE()).isHasTech((TechTypes)SAILING)) { //Rhye - no workboat explorers
...
UnitTypes CvCityAI::AI_bestUnit(bool bAsync, AdvisorTypes eIgnoreAdvisor, UnitAITypes* peBestUnitAI)
{
	...
	if (settlersModifiers[getID()][0] >= 100 && GET_PLAYER(getOwnerINLINE()).getNumCities() >= settlersCoords[getID()][6]){
			aiUnitAIVal[UNITAI_SETTLE] *= 2;
...
UnitTypes CvCityAI::AI_bestUnitAI(UnitAITypes eUnitAI, bool bAsync, AdvisorTypes eIgnoreAdvisor)
{
	...
	for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		//Rhye - start (no workboat explorers)
		//if (iI == 44 && eUnitAI == UNITAI_EXPLORE_SEA) {
		//	continue;
		//}
		//Rhye - end
		...
		//Rhye - start (no workboat explorers)
		if (eLoopUnit == 89 && eUnitAI == UNITAI_EXPLORE_SEA)
			continue;
		//Rhye - end
							...
							if (iValue > 0)
							{
								...
								if (isWorldWonderClass((BuildingClassTypes)iI))
								{
									...
									if (getOwnerINLINE() >= VIKING) //since it is now enabled for Medieval civs too
										if (iI == GREATWALL) iTempValue /= 10;  //lower since there is no 600AD start
