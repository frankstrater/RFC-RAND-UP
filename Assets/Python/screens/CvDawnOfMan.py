...
##Rhye - begin	
		...
		year = CyGameTextMgr().getDateStr(gc.getActivePlayer().getBirthTurn(), False, 0, -3000, 0)
...
##Rhye - begin  #RFCRAND
		if (gc.getActivePlayer().getBirthTurn() == 0): 
			...
			iNumAutoPlayTurns = gc.getActivePlayer().getBirthTurn() #RFCRAND
			...
			if (gc.getPlayer(0).isPlayable()):  #late start condition
                                ...
                                screen.setBarPercentage("ProgressBar", InfoBarTypes.INFOBAR_STORED, float(math.pow(iGameTurn-181, exponent)) / float(math.pow(iNumAutoPlayTurns-181, exponent)))
